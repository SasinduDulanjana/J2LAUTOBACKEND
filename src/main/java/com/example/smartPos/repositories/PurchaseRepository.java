...
public interface PurchaseRepository extends JpaRepository<Purchase, Integer> {
    ...
    @Query("SELECT TO_CHAR(p.addedDate, 'YYYY-MM') AS month, COALESCE(SUM(p.totalCost), 0) AS purchases " +
            "FROM Purchase p WHERE p.status = 1 GROUP BY TO_CHAR(p.addedDate, 'YYYY-MM') ORDER BY month")
