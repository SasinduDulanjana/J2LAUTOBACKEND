...
public interface SaleRepository extends JpaRepository<Sale, Integer> {
    ...
    @Query("SELECT TO_CHAR(s.addedDate, 'YYYY-MM') AS month, COALESCE(SUM(s.totalAmount), 0) AS revenue " +
            "FROM Sale s WHERE s.status = 1 GROUP BY TO_CHAR(s.addedDate, 'YYYY-MM') ORDER BY month")
